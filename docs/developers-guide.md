...
coverage as the trunk's. Its concurrency group never cancels a run in progress;
a newer push replaces any pending run, so the newest baseline wins. No other
workflow a push starts, directly or through a local call, may generate coverage
outside the pull-request guard, so the publisher is the only baseline writer.
